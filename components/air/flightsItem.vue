...
export default {
  ...
  computed: {
    rankTime() {
      ...
      //如果到达的时间小于出发时间，则说明到了第二天，到达小时需要加24
      if (arrTime[0] < depTime[0]) {
        arrTime[0] += 24;
      }

      //到达时间的分钟
      const arrVal = arrTime[0] * 60 + +arrTime[1];
      //出发时间的分钟
      const depVal = depTime[0] * 60 + +depTime[1];

      //相隔的总分钟
      const disTime = arrVal - depVal;

      //向下取整获取小时
      const hours = Math.floor(disTime / 60);
      //分钟
